...
void DlgProcessDetail::InitProcessDetail()
{
	...
	PROCESS_INFO* pProcessInfo = theSystemInfo->GetProcessInfo()->GetInfo(m_ulProcessID);
	...
	wxString strCommandLine(wxT(""));
	if(theSystemInfo->GetProcessInfo()->GetCommandLineOfProcess(m_ulProcessID, strCommandLine))
		m_staticCommandLine->SetLabelText(strCommandLine);
	...
	if( EnumProcessModulesEx(hProcess, hMods, sizeof(hMods), &cbNeeded, dwFlags))
    {
        ...
        for ( i = 0; i < (cbNeeded / sizeof(HMODULE)); i++ )
        {
            ...
            if ( GetModuleFileNameEx( hProcess, hMods[i], szModName, sizeof(szModName) / sizeof(wchar_t)))
            {
				...
				iIndex++;
			//	wxTreeItemId item = m_treeCtrlDetail->AppendItem(treeItemRoot, strDisp, iIconIndex, iIconIndex);
			//	m_treeCtrlDetail->SetItemTextColour(item, col);
            }
        }

	//	m_treeCtrlDetail->ExpandAll();
    }
